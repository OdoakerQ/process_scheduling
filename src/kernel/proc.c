...
PUBLIC int currGroup = PROC_GROUP_A;
 ...
 */
  int tempProc;
  int i;
  struct proc* tempProcPtr;
  struct proc* prevProcPtr;
  struct proc* foundProcPtr = NIL_PROC;

  int counter;

  if (rdy_head[USER_Q] == NIL_PROC) return;

    for (i = 0; i < PROC_GROUPS_NUM && foundProcPtr == NIL_PROC; i++) {
      tempProcPtr = rdy_head[USER_Q];
      currGroup = (currGroup + 1) % PROC_GROUPS_NUM;
      while((tempProcPtr) != NIL_PROC && foundProcPtr == NIL_PROC) {
        if (tempProcPtr->p_group == currGroup)
          foundProcPtr = tempProcPtr;

        if (foundProcPtr == NIL_PROC)
          prevProcPtr = tempProcPtr;
        tempProcPtr = tempProcPtr->p_nextready;
      }
    }

    if (foundProcPtr != NIL_PROC && foundProcPtr != rdy_head[USER_Q]) {
      prevProcPtr->p_nextready = foundProcPtr->p_nextready;
      foundProcPtr->p_nextready = rdy_head[USER_Q];
      rdy_head[USER_Q] = foundProcPtr;
      if (foundProcPtr->p_pid == rdy_tail[USER_Q]->p_pid) {
        rdy_tail[USER_Q] = prevProcPtr;
      }
    }

 ...
 */
  ...
  if (rdy_head[USER_Q] == NIL_PROC) return;
    ...
    for (i = 0; i < PROC_GROUPS_NUM; i++) {
      tempProcPtr = rdy_head[USER_Q];
      tempCurrGroup = (currGroup + i) % PROC_GROUPS_NUM;
      while((tempProcPtr = tempProcPtr->p_nextready) != NIL_PROC) {
        if (tempProcPtr->p_group == tempCurrGroup) {
          foundProcPtr = tempProcPtr;
          currGroup = (tempCurrGroup + 1) % PROC_GROUPS_NUM;
          break;
        }
      }
  ...
  if (foundProcPtr != NIL_PROC)
   	while (foundProcPtr != rdy_head[USER_Q]) {

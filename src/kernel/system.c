...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_SETGPROC: r = do_setgproc(&m); break;
      case SYS_GETGPROC: r = do_getgproc(&m); break;
      case SYS_SETGPARAM: r = do_setgparam(&m); break;
      case SYS_GETGPARAM: r = do_getgparam(&m); break;
	    default:		r = E_BAD_FCN;
	}
 ...
 *				do_setgproc				    *
 *==========================================================================*/
PRIVATE int do_setgproc(m_ptr)
register message* m_ptr;
{
    int i;
    int pid = m_ptr->m1_i1;
    int newGroup = m_ptr->m1_i2;
    struct proc* proc_ptr;

    /* printf("\nIN_SYSTEM.c(%d)\n", pid); */
  	for(i = NR_TASKS; i < NR_TASKS + NR_PROCS; i++) {
        /* printf("i:%d, prio:%d, pid: %d ", i, proc[i].p_priority, proc[i].p_pid); */
		    if(!(proc[i].p_priority == PPRI_TASK || proc[i].p_priority == PPRI_SERVER)) {
          if (proc[i].p_pid == pid) {
          proc_ptr = &proc[i];
          /* printf("\nIN_IF(%d, %d)\n", pid, proc_ptr->p_group); */
          proc_ptr->p_group = newGroup;
          return OK;
          }
        }
    }
...
register message* m_ptr;
{
  ...
  /* printf("\nIN_SYSTEM.c(%d)\n", pid); */
  	for(i = NR_TASKS; i < NR_TASKS + NR_PROCS; i++) {
  		/* printf("i:%d, prio:%d, pid: %d ", i, proc[i].p_priority, proc[i].p_pid); */
		    if(proc[i].p_priority != 0 && proc[i].p_pid == pid && &proc[i] != NIL_PROC) {
          /* printf("\nIN_IF(%d)\n", pid); */
          proc_ptr = &proc[i];
          return proc_ptr->p_group;
        }
      }
    return EINVAL;
 ...
 *				do_setgparam				    *
 *==========================================================================*/
PRIVATE int do_setgparam(m_ptr)
register message* m_ptr;
{
  int group = m_ptr->m1_i1;
  int value = m_ptr->m1_i2;

  groupParams[group] = value;
 ...
 *				do_getgparam				    *
 *==========================================================================*/
PRIVATE int do_getgparam(m_ptr)
register message* m_ptr;
{
  int group = m_ptr->m1_i1;

  return groupParams[group];
}
